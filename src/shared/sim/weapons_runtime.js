...
// then 0.55s gravitational pull dragging all enemies in pullRadius inward, then
// a crushing impact blast at 0.7s. Pull queued to g.pendingPulls; impact queued
// to g.meteorEffects with warnLife:0.7 so the render normalises correctly.
function fireVoidAnchor(g, w, p) {
  emit(g, EVT.WEAPON_FIRE, { weapon: 'void_anchor', pid: p.id });

  // Chain-style opener — hit nearest enemy immediately
  ...
  // Crushing impact after pull resolves (0.7s). warnLife lets the renderer
  // normalise the streak without assuming the standard 0.5s meteor delay.
